...
| Tenants | URL methods are tenant-scoped by default; `strict=True` requires the configured site. Path/object workflows retain the configured site/drive boundary; create separate managers for separate tenants |
...
| Latency target | Measure non-transfer Graph operations at p95 ≤ 1.5 seconds on a controlled test tenant before increasing concurrency |
...
Graph/network SLO. A controlled test-tenant run must supply the latency,
transfer, throttling, failure-recovery, and tenant-capacity evidence before
changing the sequential default or adding async APIs, queues, caches, or
broader frameworks.
